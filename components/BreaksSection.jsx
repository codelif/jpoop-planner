...
export function BreaksSection({ breaks }) {
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold text-card-foreground">Breaks in the Day</h3>
      {!breaks?.length ? (
        ...
        <ul className="space-y-2 text-sm">
          {breaks.map((b, i) => (
            <li key={i} className="p-3 rounded-xl bg-card border border-border flex flex-col gap-1">
