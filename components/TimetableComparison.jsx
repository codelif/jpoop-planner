...
function BatchSelector({ 
  ...
  const phasesForCourseSem = course && semester ? metadata?.phases?.[course]?.[semester] || [] : []
  const batchesForCourseSemPhase = course && semester && phase 
    ? metadata?.batches?.[course]?.[semester]?.[phase] || [] 
    : []

  const bgColorClass = colorScheme === "primary" ? "bg-primary/10" : 
...
function DayComparisonView({ day, batchesData, batchLabels }) {
  const allBatchBreaks = batchesData.map(data => calculateBreaks(data[day] || []))
  ...
  return (
    <div className="space-y-6">
        ...
        {coincidingBreaks.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {coincidingBreaks.map((brk, i) => (
              <div key={i} className="p-4 bg-accent/30 border border-accent/40 rounded-lg text-center">
                <div className="text-lg font-bold text-accent-foreground">{brk.start} - {brk.end}</div>
                <div className="text-sm text-muted-foreground">{brk.durationReadable}</div>
                <div className="text-xs text-muted-foreground mt-1">All batches free</div>
              </div>
            ))}
          </div>
        )}
      ...
      <div className={`grid gap-6 ${
        batchesData.length === 1 ? 'grid-cols-1' :
        batchesData.length === 2 ? 'grid-cols-1 lg:grid-cols-2' :
        batchesData.length === 3 ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3' :
            ...
            <div key={index} className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                {classes.length === 0 ? (
                  ...
                  classes.map((cls, i) => (
                    <div key={i} className="p-3 bg-card border border-border rounded-md">
                      <div className="font-semibold text-card-foreground">{cls.subject}</div>
                      <div className="text-sm text-muted-foreground">{cls.start} - {cls.end}</div>
...
export function TimetableComparison({ metadata, onClose }) {
  ...
  }, [batches.map(b => `${b.course}-${b.semester}-${b.phase}-${b.batch}`).join(',')])

  const getBatchLabel = (course, semester, phase, batch) => {
    const batchData = metadata?.batches?.[course]?.[semester]?.[phase]?.find(b => b.id === batch)
        ...
        <div className="overflow-y-auto max-h-[calc(90vh-80px)]">
          <div className="p-6 space-y-6">
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold text-foreground">Select Batches to Compare</h3>
                <Button
                  onClick={addBatch}
                  variant="outline"
                  size="sm"
                  ...
                  <BatchSelector
                    ...
                    onCourseChange={(val) => {
                      ...
                      if (firstSem) {
                        ...
                        const firstPhase = (metadata?.phases?.[val]?.[firstSem.id] || [])[0]
                        if (firstPhase) {
                          updateBatch(index, 'phase', firstPhase.id)
                          const firstBatch = (metadata?.batches?.[val]?.[firstSem.id]?.[firstPhase.id] || [])[0]
                          if (firstBatch) updateBatch(index, 'batch', firstBatch.id)
                        }
                      }
                    }}
                    onSemesterChange={(val) => {
                      updateBatch(index, 'semester', val)
                      const firstPhase = (metadata?.phases?.[batch.course]?.[val] || [])[0]
                      if (firstPhase) {
                        updateBatch(index, 'phase', firstPhase.id)
                        const firstBatch = (metadata?.batches?.[batch.course]?.[val]?.[firstPhase.id] || [])[0]
                        if (firstBatch) updateBatch(index, 'batch', firstBatch.id)
                      }
                    }}
                    onPhaseChange={(val) => {
                      updateBatch(index, 'phase', val)
                      const firstBatch = (metadata?.batches?.[batch.course]?.[batch.semester]?.[val] || [])[0]
                      if (firstBatch) updateBatch(index, 'batch', firstBatch.id)
                    }}
            ...
            {canCompare && (
              <DayComparisonView
                ...
                batchLabels={validBatches.map(batch => 
                  getBatchLabel(batch.course, batch.semester, batch.phase, batch.batch)
                )}
              />
            )}
            ...
            {!canCompare && !isAnyLoading && (
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-semibold text-foreground mb-2">Select At Least Two Batches to Compare</h3>
                <p className="text-muted-foreground">Choose course, semester, phase, and batch for multiple sides to start comparing timetables.</p>
              </div>
            )}
          </div>
        </div>
